...
struct PCB{
    ...
    unsigned int priority;  // Added for External Priority scheduling
    unsigned int time_in_io;  // Track I/O time
    unsigned int last_io_time;  // Track when last I/O occurred
};
...
PCB add_process(std::vector<std::string> tokens) {
    ...
    process.priority = (tokens.size() > 6) ? std::stoi(tokens[6]) : 0;  // Optional priority field
    process.start_time = -1;
    process.partition_number = -1;
    process.state = NOT_ASSIGNED;
    process.time_in_io = 0;
    process.last_io_time = 0;
    return process;
}
